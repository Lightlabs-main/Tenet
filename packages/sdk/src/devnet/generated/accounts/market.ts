...
export const MARKET_DISCRIMINATOR: ReadonlyUint8Array = new Uint8Array([219, 190, 213, 55, 0, 227, 198, 154]);

export function getMarketDiscriminatorBytes(): ReadonlyUint8Array { return fixEncoderSize(getBytesEncoder(), 8).encode(MARKET_DISCRIMINATOR); }

export type Market = { discriminator: ReadonlyUint8Array; mint: Address; tokenProgram: Address; feed: Address; inventory: Address; usdcVault: Address; spreadBps: number; bump: number;  };

export type MarketArgs = { mint: Address; tokenProgram: Address; feed: Address; inventory: Address; usdcVault: Address; spreadBps: number; bump: number;  };

/** Gets the encoder for {@link MarketArgs} account data. */
export function getMarketEncoder(): FixedSizeEncoder<MarketArgs> {
    return transformEncoder(getStructEncoder([['discriminator', fixEncoderSize(getBytesEncoder(), 8)], ['mint', getAddressEncoder()], ['tokenProgram', getAddressEncoder()], ['feed', getAddressEncoder()], ['inventory', getAddressEncoder()], ['usdcVault', getAddressEncoder()], ['spreadBps', getU16Encoder()], ['bump', getU8Encoder()]]), (value) => ({ ...value, discriminator: MARKET_DISCRIMINATOR }));
}

/** Gets the decoder for {@link Market} account data. */
export function getMarketDecoder(): FixedSizeDecoder<Market> {
    return getStructDecoder([['discriminator', fixDecoderSize(getBytesDecoder(), 8)], ['mint', getAddressDecoder()], ['tokenProgram', getAddressDecoder()], ['feed', getAddressDecoder()], ['inventory', getAddressDecoder()], ['usdcVault', getAddressDecoder()], ['spreadBps', getU16Decoder()], ['bump', getU8Decoder()]]);
}
