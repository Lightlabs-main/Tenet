...
|---|---|---|
| Terminology and product loop | `README.md`, `docs/architecture.md`, Rust state names, SDK names, UI labels | compliant; Fork/amendment behavior is still future work, but terms are reserved correctly |
...
| Instruction surface | `programs/tenet/src/lib.rs` | `cancel_contribution`, `refresh_asset_metadata`, Fork source handlers and staged redemption exist; execution and amendments do not; generated IDL/deployable artifact is stale | Preserve stable names; regenerate and verify `refresh_asset_metadata`/Fork, then add the execution state machine and amendment instructions. Keep begin/end execution if required to verify post-swap deltas | add-only account/instruction migration; no rename of existing deployed instruction | refresh account-validation tests, Fork tests, execution replay tests |
...
| Fork | `Mandate.forked_from`, `fork_mandate` and `fork_mandate_asset` now exist in source; IDL/deploy regeneration pending | Child constitution path is source-complete but not yet deployed in this environment | Copy rules/config/lineage only; child asset PDAs are copied independently; parent read-only and capital untouched | add-only PDAs/instructions; deployed parent accounts remain unchanged | `test_fork_copies_rules`, `test_fork_does_not_move_parent_assets`, `test_child_mandate_cannot_modify_parent` |
| Amendments | threshold/delay fields exist in `Mandate`; no proposal/vote/execute accounts/instructions | Architecture only; UI must not fake it | Implement threshold + delay governance after core mechanics; exit/fork remain alternatives | add proposal/vote PDAs; no mutation of old Mandate layout without migration | `test_amendment_requires_threshold_and_delay` |
...
| Required named tests | Rust/domain suites | Fork names now exist in the program test source; amendments, execution, and automation names do not | Add exact canonical names or explicit aliases only when behavior is identical; preserve all existing tests | none | full Rust/TS suite |
...
6. Complete Fork integration verification, then implement amendment governance.
